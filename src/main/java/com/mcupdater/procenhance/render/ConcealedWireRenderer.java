...
public class ConcealedWireRenderer implements BlockEntityRenderer<ConcealedWireEntity> {
	private BlockRenderDispatcher blockRenderer;

	public ConcealedWireRenderer(BlockEntityRendererProvider.Context context) {
	...
	public void render(ConcealedWireEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		BlockState renderState = blockEntity.mimic;
		LocalPlayer player = Minecraft.getInstance().player;
		ItemStack mainHand = player.getMainHandItem();
		if (player.isDiscrete() && !mainHand.isEmpty() && mainHand.is(Tags.Items.TOOLS_WRENCH)) renderState = Registration.CONCEALEDWIRE_BLOCK.get().defaultBlockState();
