...
public class PatternCrossItem extends AbstractPatternItem {
    private static final Map<Ingredient, CraftingRecipe> RECIPES = new HashMap<>();
    private static final Set<Integer> exclusions = Set.of(0,2,6,8);
    public PatternCrossItem(Properties pProperties) {
        super(pProperties);
        ...
        if (stack.getCount() >= ingredientCount) {
            ...
            if (!RECIPES.containsKey(ingredient)) {
                ...
                for (int slot = 0; slot < 9; slot++) {
                    testContainer.setItem(slot, (exclusions.contains(slot) ? ItemStack.EMPTY : testStack));
                }
    ...
    public ItemStack cycleItem() {
        return new ItemStack(Registration.SLATE_3x3.get(),1);
    }
}
