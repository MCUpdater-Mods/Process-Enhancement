...
import com.mcupdater.procenhance.util.TagUtil;
...
public class TagRecipeResult extends RecipeResult {
	...
	public ItemStack[] getItemStacks() {
		if (this.cachedItemStacks == null) {
			this.cachedItemStacks = BuiltInRegistries.ITEM.getTag(this.tag)
	...
	public RecipeResultType<?> getType() {
		return Registration.RESULT_TAG.get();
	}

	@Override
	public RecipeResult copyWithCount(int count) {
		return new TagRecipeResult(this.tag, this.count, this.patch);
	}
}
